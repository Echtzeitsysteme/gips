...
class GipsAPITemplate extends ProblemGeneratorTemplate<GipsIntermediateModel> {
	...
	override init() {
		...
		data.model.mappings.map[m|data.mapping2mapperClassName.get(m)].forEach [ m |
			imports.add(data.apiData.gipsMapperPkg + "." + m)
		]
...
//	}
	...
	override String generateValueAccess(ValueExpression expression, boolean requiresReturn) {
		...
		if(expression instanceof MappingReference) {
			...
			instruction = super.generateValueAccess(expression, requiresReturn)

		} else {
			// CASE: ContextReference
			instruction = generateIterator(expression as ContextReference)
		}
		return instruction;
	}
