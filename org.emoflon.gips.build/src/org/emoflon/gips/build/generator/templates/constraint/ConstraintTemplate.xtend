	...
	def void computeVariableTermBuilder(ArithmeticExpression expr, LinkedList<String> methodCalls) {
		if(expr instanceof ArithmeticBinaryExpression) {
			if(expr.operator == ArithmeticBinaryOperator.ADD) {
				...
				val instruction = '''terms.add(new Term(«generateVariableAccess(variable.iterator.next)», «builderMethodName»(«getCallContextParameter()»«IF !getConstants().empty», «ENDIF»«getCallParametersForConstants(getConstants())»)));'''
