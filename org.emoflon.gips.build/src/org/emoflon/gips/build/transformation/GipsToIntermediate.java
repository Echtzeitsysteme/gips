...
public class GipsToIntermediate {
	...
	protected void transformMappings() {
		data.gipslFile().getMappings().forEach(eMapping -> {
			...
			if (eMapping.getPattern().getType() == GTLRuleType.RULE) {
				...
				ruleMapping.setContextPattern((GTPattern) ruleMapping.getRule().getPrecondition());
				mapping = ruleMapping;
				...
				GTPattern pattern = data.getPattern(eMapping.getPattern());
				pmMapping.setContextPattern(pattern);
	...
	protected void transformMappingVariables(GipsMapping mapping, RuleMapping ruleMapping) {
		...
		for (GipsMappingVariable gipsVar : mapping.getVariables()) {
			if (gipsVar.isBound()) {
				...
				GTRule rule = data.getRule(mapping.getPattern());
				var.setRule(rule);
	...
	protected Constraint createConstraint(final GipsConstraint eConstraint, int counter) {
		if (eConstraint.getContext() instanceof GipsMapping mapping) {
			...
			return constraint;
		} else if (eConstraint.getContext() instanceof SlimRule ePattern) {
			if (ePattern.getType() == GTLRuleType.RULE) {
				GTRule rule = data.getRule(ePattern);
				...
				constraint.setContextPattern((GTPattern) rule.getPrecondition());
				constraint.setGlobal(false);
				return constraint;
			} else {
				GTPattern pattern = data.getPattern(ePattern);
	...
	protected Constraint createDisjunctConstraint(final GipsAnnotatedConstraint eConstraint, int counter) {
		if (eConstraint.input().getContext() instanceof GipsMapping mapping) {
			...
			constraint.setGlobal(false);
			return constraint;
		} else if (eConstraint.input().getContext() instanceof SlimRule ePattern) {
			if (ePattern.getType() == GTLRuleType.RULE) {
				GTRule rule = data.getRule(ePattern);
				RuleConstraint constraint = factory.createRuleConstraint();
	...
	protected LinearFunction createLinearFunction(final GipsLinearFunction eLinearFunction) {
		if (eLinearFunction.getContext() instanceof GipsMapping mapping) {
			...
			return function;
		} else if (eLinearFunction.getContext() instanceof SlimRule ePattern) {
			if (ePattern.getType() == GTLRuleType.RULE) {
				GTRule rule = data.getRule(ePattern);
				RuleFunction function = factory.createRuleFunction();
				function.setRule(rule);
				function.setContextPattern((GTPattern) rule.getPrecondition());
				function.setName(eLinearFunction.getName());
				return function;
			} else {
				GTPattern pattern = data.getPattern(ePattern);
	...
	protected Constant createConstant(final GipsConstant eConstant, final Context context) throws Exception {
		...
		if (eConstant.eContainer() instanceof EditorFile)
	...
	protected void mapGT2IBeXElements() {
		data.gipslFile().getRules().stream().filter(r -> r.getType() == GTLRuleType.RULE).forEach(eRule -> {
			for (GTRule rule : data.model().getIbexModel().getRuleSet().getRules()) {
				if (rule.getName().equals(eRule.getName())) {
					data.addRule(eRule, rule);
				}
			}
		});

		// Add all patterns
		for (SlimRule ePattern : data.gipslFile().getRules()) {
			for (GTPattern pattern : data.model().getIbexModel().getPatternSet().getPatterns().stream()
					.map(p -> (GTPattern) p).collect(Collectors.toList())) {
				if (pattern.getName().equals(ePattern.getName())) {
					data.addPattern(ePattern, pattern);
					for (IBeXNode node : pattern.getSignatureNodes()) {
						ePattern.getContextNodes().stream().map(eNode -> (SlimRuleNode) eNode.getContext())
								.forEach(eNode -> {
									if (eNode.getName().equals(node.getName())) {
										data.eNode2Node().putIfAbsent(eNode, node);
									}
								});
						ePattern.getCreatedNodes().stream().map(eNode -> (SlimRuleNode) eNode.getCreation())
								.forEach(eNode -> {
									if (eNode.getName().equals(node.getName())) {
										data.eNode2Node().putIfAbsent(eNode, node);
									}
								});
						ePattern.getDeletedNodes().stream().map(eNode -> (SlimRuleNode) eNode.getDeletion())
								.forEach(eNode -> {
									if (eNode.getName().equals(node.getName())) {
										data.eNode2Node().putIfAbsent(eNode, node);
									}
